  ...
  return (
    <div className="m-3 max-w-sm bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
        <Link to={`/products/${id}`} className="relative" >
            { best_seller && <span className="absolute top-4 left-2 px-2 bg-orange-500 opacity-90 text-white rounded">Best Seller</span> }
        ...
        <div className="p-5">
            ...
            <p className="flex justify-between items-center">
                ...
                { !inCart && <button onClick={() => addHandler(product)} className={`inline-flex items-center py-2 px-3 text-sm cursor-pointer gap-2 font-medium text-center text-white bg-rose-600 rounded-lg hover:bg-rose-700 ${isInStock ? "" : "cursor-not-allowed"}`} disabled={!isInStock}>Add To Cart <IoMdAdd size={20} /></button> }  
